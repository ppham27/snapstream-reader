...
int main(int argc, char **argv) {
  ...
  char *query; char *search_string1; char *search_string2;
  char file_name[100];
  file_name[0] = '\0';
  char *date;
  query = strtok(query_string, "&");
  while (query != NULL) {
    if (strstr(query, "search-string-1=") == query) {      
      search_string1 = strchr(query, '=') + 1;
    } else if (strstr(query, "search-string-2=") == query) {
      search_string2 = strchr(query, '=') + 1;      
    } else if (strstr(query, "date=") == query) {      
      date = strchr(query, '=') + 1;      
    }
    query = strtok(NULL, "&");
  }
  strcat(file_name, "../../legacy/build/Data/");
  strcat(file_name, date);
  strcat(file_name, "-Combined.txt");
  printf("Search string 1: %s\n", search_string1);
  printf("Search string 2: %s\n", search_string2);
  printf("File name: %s\n", file_name);

  int i,j,t;
  loadtitles();
  LEN = getstring(file_name);
  ...
  for (i=0;i<numprogs;i++) {
    if (titlenums[i] != 0) {
      ...
      /* find(i, argv[2], sec); */
      /* t = and(pri, sec); */
      t = near(i, 50, search_string1, search_string2, pri);
      /* find(i,"nafta",sec); */
      /* t = or(pri,sec); */
      /* t = or(pri,sec); */
