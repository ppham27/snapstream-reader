...
var storageFolder = 'daily_2016_presidential';
...
candidates = candidates.split('\n').map(function(candidate) { 
               ...
               if (searchTerm && splitLine[3] === "2016 Presidential Election") candidatesDict[splitLine[0]] = {searchTerm: splitLine[1], symbol: splitLine[2]};
               return splitLine[3] === "2016 Presidential Election" ? searchTerm : undefined;
...
if (readFile) {
  ...
  var req = http.request(requestOptions, function(res) {
              ...
              res.on('end', function() {
                processResponse(responseBody);              
                fs.writeFileSync(upperBoundDate.toISOString().slice(0, 10) + '_2016_presidential.html', responseBody);
              });
            });
  req.write(postData);
  req.end();
...
function processResponse(body) {
  ...
  async.waterfall([
    ...
    function(candidateData, datesArray, done) {
      ...
      var htmlEmail = '<p>The number of matching contexts for 2016 Presidential Election are <table style="border-collapse: collapse"><thead><tr>\n\
...
<td style="' + style + '">' + d["Search Term"] + '</td>\n';
      ...
      var query = {title: '2016 Presidential Election from ' + fromDateString + ' to ' + toDateString,
      ...
      transporter.sendMail({
        from: mailFrom,
        to: mailTo,
        subject: '2016 Presidential Election from ' + fromDateString + ' to ' + toDateString,
        html: htmlEmail
      }, function(err, info) {
